...
def mostrar_menu(pantlla:pygame.Surface,cola_eventos:list[pygame.event.Event]) -> str:
    ...
    pantlla.blit(fondo_pantalla,(0,0))
        # Posicionar la imagen de fondo del título (centrada horizontalmente)
    titulo_x = (PANTALLA[0] - fondo_titulo.get_width()) // 2
    titulo_y = 10  # Ajusta la posición vertical
    pantlla.blit(fondo_titulo, (titulo_x, titulo_y))
    
    # Mostrar el texto del título centrado sobre la imagen
    # Define una nueva fuente con tamaño más grande (ej: 60 píxeles)  # Añadido "bold" para negrita
    titulo = FUENTE_TITULO.render("QUIZ DE FUTBOL", True, COLOR_BLANCO)
    titulo_rect = titulo.get_rect(center=(PANTALLA[0] // 2, 55))  # Centrado en X, posición Y=100
    ...
    mostrar_texto(lista_botones[BOTON_CONFIG]["superficie"],"AJUSTES",(0,0),FUENTE_BOTONES,COLOR_BLANCO)
