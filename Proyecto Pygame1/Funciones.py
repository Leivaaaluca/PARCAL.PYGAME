...
def verificar_respuesta(datos_juego:dict,pregunta_actual:dict,respuesta:int, comodines=None) -> bool:
    if pregunta_actual["correcta"] == respuesta:
        ...
        # Incrementar contador de correctas seguidas
        datos_juego["correctas_seguidas"] += 1

        # Verificar si llega a 5 aciertos seguidos
        if datos_juego["correctas_seguidas"] >= 5:
            datos_juego["tiempo_restante"] += 10  # suma 10 seg
            datos_juego["correctas_seguidas"] = 0  # reinicia el contador
            print("¡Bonus de 10 segundos por 5 aciertos seguidos!")

        retorno = True

    else:
        # Reinicia el contador al fallar
